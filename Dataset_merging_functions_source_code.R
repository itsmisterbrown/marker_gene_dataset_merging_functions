...
merge.dataset <- function(..., standardization="relative", return.all=FALSE){
  ...
  if (standardization=="geomeans"){
    ...
    sampleSums <- sapply(phy.list, FUN = function(x) phyloseq::sample_sums(x))
    run.geomeans <- geomeans.r(runSums)
    sample.geomeans <- geomeans.r(c(unlist(sampleSums)))
    ...
    sample.median <- median(c(unlist(sampleSums)))
  ...
  if (return.all==FALSE){
    ...
    l.return[['datasets.merged']] <- dataset.count
